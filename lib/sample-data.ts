...
const quikPdf = (payload?: string): EndpointConfig => ({
  id: "ep-quik", name: "Quik PDF", type: "quik_pdf", endpointUsage: "pdf",
  payloadTemplateJson: payload ?? JSON.stringify({}, null, 2),
});
const pershingApi = (payload?: string): EndpointConfig => ({
  id: "ep-pershing", name: "Pershing API", type: "pershing_api", endpointUsage: "action",
...
export const CUSTODIAN_FORM_RULES: CustodianFormRule[] = [
  {
    ...
    endpoints: [quikPdf(FIDELITY_BROKERAGE_PAYLOAD)],
    enabled: true,
  ...
  },
  {
    ...
    eSignatureMappingJson: FIDELITY_ESIG,
    endpoints: [quikPdf(FIDELITY_IRA_PAYLOAD)],
    enabled: true,
  ...
  },
  {
    ...
    endpoints: [quikPdf(SCHWAB_PAYLOAD)],
    enabled: true,
  ...
  },
  {
    ...
    endpoints: [quikPdf(PERSHING_PAYLOAD), pershingApi()],
    enabled: true,
